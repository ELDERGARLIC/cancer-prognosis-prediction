...
TCGA-BRCA pipeline at +0.072 C-index between leaky and per-fold-honest
...
approximately +0.013 attributable to the gene graph itself and the
remainder to the non-linear MLP head's use of clinical features. The
+0.013 gene-graph contribution is small but stable to within bootstrap
...
the worked example, presents the small-but-stable +0.013 gene-graph
...
A 769-gene candidate set, hereafter the *prior gene universe*, was
...
Seven TCGA clinical features were retained for internal-only analyses:
age (z-scored), stage_ordinal (0–1 normalized), one-hot stage indicators
(stage_I/II/III/IV), and is_female. ER and PR binary status columns in
the legacy preprocessed file (`er_signed`, `pr_signed`) contain all-zero
values, a preprocessing artifact inherited from the prior pipeline.
Recovery from raw clinical records is documented in Discussion §future-work;
reported clinical contributions accordingly understate ER/PR-inclusive
performance by an estimated +0.02–0.04 C-index.
...
within-TCGA results use the 7-feature variant.
...
**Cox PH HONEST (north-star, 0.6605 ± 0.014).** The methodologically
...
per-fold-z-normalized expression of those genes plus the 7 clinical
...
**Cox PH LEAKY (upper bound, 0.7324 ± 0.014).** The same Cox PH formulation
applied to the prior 769-gene universe. Reported as an upper bound because
its gene selection peeked at every patient's label including those in
val folds. The +0.072 c-index gap between LEAKY and HONEST is the leakage-
...
**MLP clinical-only (non-linear flat-feature reference, 0.7122 ± 0.038).**
A two-layer MLP (Linear-128 → ReLU → Dropout-0.4 → Linear-1) on the
7-clinical-feature vector trained with the same Cox partial-likelihood
...
clinical feature vector (7-dim internally, 3-dim for METABRIC-compatible
...
fold-matched Cox PH (PCA(100) + 7 clinical, penalizer 0.5) was fit and
evaluated on the held-out val patients. Across folds, the per-fold-honest
Cox PH achieved 0.6605 ± 0.049 (mean C-index ± std across folds), versus
the leaky-cohort Cox PH at 0.7324 ± 0.014 — a +0.072 c-index leakage cost
on average. This audit established Cox PH HONEST = 0.6605 as the
...
**Clinical feature alignment.** TCGA's 7-feature clinical vector is
...
[repository URL placeholder]; the full pipeline can be reproduced
...
as the source of an upper-bound bias. The 769-gene candidate set was
...
Cox PH (PCA-100 + 7 clinical features, ridge penalizer 0.5) was fit on
each fold's per-fold gene set and evaluated on its held-out val patients.

The per-fold-honest Cox PH achieves a mean val C-index of 0.6605
(std 0.049 across the 5 folds), versus the leaky-cohort Cox PH at 0.7324
(std 0.014). The +0.072 c-index leakage cost is the headline finding of
the Stage 0 audit and establishes Cox PH HONEST = 0.6605 as the
...
|---|---|---:|---:|---|
| Cox PH HONEST | per-fold LASSO on raw 60k matrix | **0.6605** | 0.049 | North-star; the gate any GNN claim must clear |
| Cox PH LEAKY | full-cohort LASSO (legacy pipeline) | 0.7324 | 0.014 | Upper bound; reported for context only |
...
same 5 stratified splits achieves a mean val C-index of 0.7200
(std 0.043; per-fold values [0.7360, 0.7627, 0.6887, 0.7599, 0.6525]).
The R1 embedding-collapse sentinel passed across all five folds.
Knob A is +0.0595 above Cox PH HONEST on fold-mean — a clear separation
from the leakage-corrected linear baseline.

Against the leakage-corrupted upper bound (Cox PH LEAKY = 0.7324), knob A
is −0.0124 on fold-mean. Paired-bootstrap delta on identical val patients
(n_boot = 2000) gives Δ = −0.046, 95% CI [−0.103, +0.006], P(A ≤ Cox-LEAKY)
= 0.96. The 95% CI's upper edge grazes zero; knob A is statistically tied
with the leakage-corrupted upper bound on the paired-bootstrap test.

The MLP clinical-only reference (two-layer MLP on the same 7-clinical
features, Cox loss, no gene graph) achieves 0.7122 (std 0.038 across
...
non-linear MLP-clinical is +0.013 internally, stable to within bootstrap
noise across all three GNN variants (knob D: +0.013; knob A: +0.008; knob
B: +0.010), establishing a small but architecture-invariant gene-side signal.
...
precursor (knob D, the same architecture trained on the prior 769-gene
universe without per-fold LASSO) produces fold-mean Δ = −0.006 and
paired-bootstrap Δ = +0.027, 95% CI [−0.002, +0.056], P(A ≤ D) = 0.033.
...
|---|---:|---:|---:|---:|
| Cox PH HONEST | 0.6605 ± 0.049 | 0.5914 [0.571, 0.612] | — | −0.053 [−0.076, −0.031] |
| MLP clinical-only | 0.7122 ± 0.038 | not run | n/a | n/a |
| **Knob A** | **0.7200 ± 0.043** | **0.6443 [0.624, 0.664]** | reference | reference |
| Knob B (pathway pool) | 0.7222 ± 0.055 | 0.6285 [0.607, 0.648] | −0.011 [−0.036, +0.013] | **−0.016 [−0.026, −0.006]** |
| Knob C (BioBERT init) | 0.7414 (10%-val) | 0.6054 [0.583, 0.627] | n/a (no full 5-fold run) | **−0.039 [−0.059, −0.021]** |
...
and METABRIC external: knob A drops by +0.082 (5-fold mean 0.7200 →
...
The external knob A C-index of 0.6443 is within 0.02 c-index of Cox PH
HONEST on TCGA internal (0.6605). Knob A applied cold to METABRIC scores
...
and 5-year survival are −0.891 (TCGA) and −0.879 (METABRIC); 3-year
correlations are −0.782 (TCGA) and −0.745 (METABRIC). The model's
...
mean val C-index 0.7222 (std 0.055). Paired-bootstrap delta versus knob A
on identical TCGA val patients is −0.011, 95% CI [−0.036, +0.013],
P(B ≤ A) = 0.83 (CI crosses zero, statistically tied). *Result,
...
On TCGA internal validation, knob C achieves C-index 0.7414 on the 10%-
val held-out for best-epoch selection, within fold variance of knob A's
0.7492 — a tie under any reasonable internal-only assessment. On the
METABRIC external paired test, knob C scores 0.6054 with paired Δ versus
knob A of −0.0389 (95% CI [−0.059, −0.021], P(C ≤ A) = 1.000), a result
the internal comparison could not have predicted. The 5-fold internal
mean for knob C was not computed; the 10%-val number is reported as a
within-cohort point estimate, while the external paired-bootstrap test
(n = 1,466) is the primary architectural comparator (Methods §5.3).

The cohort-shift drop quantifies the asymmetry directly. Knob A drops
0.082 c-index between internal and external (0.7200 → 0.6443); knob C
drops 0.136 (0.7414 → 0.6054), approximately 1.7 times knob A's drop.
...
three panels: TCGA 10%-val C-index, METABRIC C-index with 95% CI bars,
and paired Δ versus knob A on identical METABRIC patients with CI bars.
...
![**Figure 4.** Architecture-ablation forest plot. Three panels: TCGA 10%-val C-index (left), METABRIC C-index with 95% CI bars (middle), and paired Δ versus knob A on identical METABRIC patients with CI bars (right). Triple stars (★★★) mark P(model ≤ A) ≥ 0.999 with 95% CI strictly below zero. Knob B, knob C, and Cox PH all fall significantly below knob A on the external paired test despite knob C being internally competitive.](../figures/fig4_architecture_forest.png)
...
lowest-risk val patients per fold) are: Signaling by Interleukins (4.17
total weight), Cytokine Signaling in Immune System (4.03), MAPK Family
Signaling Cascades (3.59), Interleukin-4 and Interleukin-13 Signaling
(3.59), Signaling by Receptor Tyrosine Kinases (2.52), ESR-Mediated
Signaling (1.86), Signaling by Nuclear Receptors (1.64), Signaling by
Notch (1.28), Estrogen-Dependent Gene Expression (1.16), and Platelet
Activation Signaling and Aggregation (0.94).
...
HONEST > LEAKY by 0.016 on fold 3, against the +0.072 mean leakage cost
