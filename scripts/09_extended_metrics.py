...
def write_latex(out):
    def row(name, blk, fmt="{:.3f}", pkey="p_delta_le_0", lower=False):
        ...
        # No dangling "--" glued onto the delta when no CI was computed (e.g.
        # Antolini's point-estimate row): omit the CI clause entirely rather
        # than print an unbracketed fragment, and mark a missing p with the
        # manuscript-wide "---" not-applicable convention instead of a blank
        # cell, so the column never renders ragged.
