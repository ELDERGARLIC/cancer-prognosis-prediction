...
Inputs:
  - results/stage_3b_sage_clinical_lasso_honest.json  (TCGA per-fold val predictions)
  - results/stage_5_metabric_external.json            (full-TCGA-trained METABRIC predictions)
  - data/processed/cv_splits.json                     (to verify OOF pooling is complete)
...
def pool_tcga_oof():
    ...
    for fr in d["fold_results"]:
        ...
        fold_id.append(np.full(len(fr["val_T"]), fr["fold"], dtype=int))
